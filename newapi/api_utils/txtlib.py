...
def get_one_temp_params(
    text, tempname="", templates=[], lowers=False, get_all_temps=False
):
    ...
    # ---
    temps = [
        x.replace("قالب:", "").replace("Template:", "").replace("_", " ").strip()
        for x in temps
    ]
    # ---
    if lowers:
...
if __name__ == "__main__":
    ...
    for temp in ingr:
        # ---
        name, namestrip, params, template = (
            temp["name"],
            temp["namestrip"],
            temp["params"],
            temp["item"],
